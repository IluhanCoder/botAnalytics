...
Проєкт готовий до автоматичного деплою на Render.com з підтримкою ML-моделей:

```bash
# 1. Запуш код на GitHub (вже зроблено)
# 2. На Render: New → Blueprint → Вибери репозиторій
# 3. Render автоматично створить backend + frontend
# 4. Додай змінні оточення (MONGODB_URI, JWT_SECRET)
# 5. ML-моделі завантажаться автоматично при першому запуску
```

📖 **Детальна інструкція:** [DEPLOYMENT.md](./DEPLOYMENT.md)

**Важливо:** ML-моделі (~500MB) не в git - вони завантажуються автоматично через `@xenova/transformers` і кешуються на Render persistent disk.
